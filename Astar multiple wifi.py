...
def reconstruct_time_path(came_from, end, start):
...
def algorithm(draw, grid, start, end, robot_id, max_wait=3, wait_penalty=5):
    count = 0
    ...
    while not open_set.empty():
        ...
        if current == end:
            ...
            path_nodes = reconstruct_time_path(came_from, end, start)
        ...
        for neighbor in current.neighbors:
            ...
            # Check for head-on swap: (A->B at t+1 and B->A at t)
            reverse_key = ((current.row, current.col, timen + 1), (neighbor.row, neighbor.col, timen))
            if reservation_table.get((neighbor.row, neighbor.col, timen + 1)) != robot_id and \
            reservation_table.get((current.row, current.col, timen)) != robot_id and \
            ((neighbor.row, neighbor.col, timen) in reservation_table and
                reservation_table[(neighbor.row, neighbor.col, timen)] != robot_id):
                continue  # Prevent head-on swap


            if ((neighbor.row, neighbor.col, next_time) in reservation_table and
                reservation_table[(neighbor.row, neighbor.col, next_time)] != robot_id):
                continue  # Collision detected in time-space
        ...
        # Wait logic with capped wait time and penalty
        for wait_offset in range(1, max_wait + 1):
            wait_time = timen + wait_offset
            wait_key = (current, wait_time)

            if ((current.row, current.col, wait_time) not in reservation_table or
                reservation_table[(current.row, current.col, wait_time)] == robot_id):

                temp_g_score = g_score[(current, timen)] + wait_penalty * wait_offset

                if wait_key not in g_score or temp_g_score < g_score[wait_key]:
                    came_from[wait_key] = (current, timen)
                    g_score[wait_key] = temp_g_score
                    f_score[wait_key] = temp_g_score + h(current.get_pos(), end.get_pos())
                    if wait_key not in open_set_hash:
                        count += 1
                        open_set.put((f_score[wait_key], count, wait_time, current))
                        open_set_hash.add(wait_key)
                    ...
                    if all_ready:
                        ...
                        for robot in sorted(robots, key=lambda r: r['id']): #new
                            algorithm(lambda: draw(win, grid, ROWS, width), 
                                     grid, robot['start'], robot['end'], robot['id'])
                        execute_all_pending()
